...
impl Consensus {
    ...
    fn update_validator_mode(&self, certificate: &Certificate, state: &mut State) -> Option<Certificate> {
        ...
        if state.ss_validator_sets.entry(ss_wave).or_insert(BTreeSet::new()).contains(&certificate.origin()) {
        ...
        if state.fb_validator_sets.entry(fb_wave).or_insert(BTreeSet::new()).contains(&certificate.origin()) {
        ...
        if state.ss_validator_sets.entry(ss_wave-1).or_insert(BTreeSet::new()).contains(&certificate.origin())
            && ss_leader.is_some() {
                state.ss_validator_sets.entry(ss_wave).or_insert(BTreeSet::new()).insert(certificate.origin());
                return ss_leader;
        }
        ...
        if state.fb_validator_sets.entry(max(fb_wave-1,1)).or_insert(BTreeSet::new()).contains(&certificate.origin())
            && fb_leader.is_some() {
                state.ss_validator_sets.entry(ss_wave).or_insert(BTreeSet::new()).insert(certificate.origin());
                return fb_leader;
        }
    ...
    fn order_wave_leaders(&self, leader: &Certificate, state: &mut State) -> Vec<Certificate> {
            ...
            .step_by(2)
        {
            ...
            let ss_sets = state.ss_validator_sets.entry(wave).or_insert(BTreeSet::new());
            ...
            let ss_stake: Stake = state
                ...
                .expect("Should have previous round certificates")
                .values()
                .filter(|(d, x)| leader.header.parents.contains(d) && self.linked(x, prev_ss_leader, dag))
                .filter(|(_, x)| ss_sets.contains(&x.origin()))
                .map(|(_, x)| self.committee.stake(&x.origin()))
                .sum();
            ...
            let fb_stake: Stake;
            let fb_sets = state.fb_validator_sets.entry(wave/2).or_insert(BTreeSet::new());
            ...
            if wave % 2 == 0 {
                ...
                fb_stake = state
                    ...
                    .expect("Should have previous round certificates")
                    .values()
                    .filter(|(d, x)| leader.header.parents.contains(d) && self.linked(x, prev_fb_leader, dag))
                    .filter(|(_, x)| fb_sets.contains(&x.origin()))
                    .map(|(_, x)| self.committee.stake(&x.origin()))
                    .sum();
                
                if ss_stake < self.committee.validity_threshold() && fb_stake >= self.committee.validity_threshold() {
